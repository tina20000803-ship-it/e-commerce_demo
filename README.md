...
本專案從零開始設計符合第三正規化的關聯式資料庫，利用 Python 灌入**10,000 筆以上**的模擬數據，並撰寫 SQL視窗函數進行核心商業分析，最終透過 Streamlit 打造出供決策者操作的動態圖表後台。

---

## 🚀 專案核心亮點 (Key Highlights)

* **寫入優化**：利用 Python `pyodbc` 的 `fast_executemany` 特性，將萬筆資料寫入速度提升，有效解決傳統單筆 Insert 的 I/O 瓶頸。
...
* `OrderDetails` (訂單明細表)：內含 `ActualUnitPrice`(實際購買單價），用以攔截因未來商品調價而導致歷史財務報表失真的歷史價格陷阱。
